Ext.define('customer.message.MessageProcessor', {
	...
	statics: {
		...
		sendRequest: function(request) {
			...
			header.add(nMessageHeader.SERVICE_TYPE, request.serviceType);
			...
			if (!Ext.isEmpty(statusMessage)) {
				if (!Ext.isEmpty(statusMessage.payload)) {
					...
					for (var i = 0; i < statusMessage.payload.length; i++) {
						...
						if (statusMessage.payload[i].statusType == appStatusType.STATUS_TYPE_ERROR) {
